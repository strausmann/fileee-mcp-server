...
**Wichtiger Stand für dieses Increment, vorab:** `internal/server.New` baut aus beiden Werten den `internal/issued.Store`. Die eine Hälfte ist bereits verdrahtet: **jeder erfolgreiche Lese-Aufruf** (die generischen Deskriptor-Werkzeuge über `registerReadService`/`registerSync`, sowie die zehn handgeschriebenen Werkzeuge `list_documents`, `search_documents`, `get_document`, `sync_documents`, `list_document_conversations`, `list_boxes`, `get_box` und — bewusst ohne — `get_document_pdf`/`get_page_image`/`get_page_ocr`, siehe deren eigene Doc-Kommentare) ruft `rec.Record` für jede ausgelieferte fileee-eigene ID. `FILEEE_ISSUED_ID_TTL_SECONDS` und `FILEEE_ISSUED_ID_MAX_PER_IDENTITY` greifen dabei bereits produktiv — `FILEEE_ISSUED_ID_MAX_PER_IDENTITY=0` etwa sorgt dafür, dass sich der Store tatsächlich nichts merkt, weil jede Aufnahme sofort verdrängt wird. **Noch nicht verdrahtet ist die Gegenseite:** Kein Werkzeug befragt eine übergebene ID vor dem Handeln über `rec.Check` gegen diesen Store — das ist der Schritt, den ein späteres Increment (Spec 3b, destruktive und `share`-Werkzeuge, siehe ADR-0013/ADR-0019) nachträgt. Bis dahin hat das Aufnehmen selbst noch keine schützende Wirkung; es legt nur die Grundlage, gegen die eine künftige Prüfung greifen kann.
