...
**Wichtiger Stand für dieses Increment, vorab:** `internal/server.New` baut aus beiden Werten bereits den `internal/issued.Store`, der sie tatsächlich auswertet (`Record`/`Check`) — aber noch **kein** Lese- oder Schreib-Werkzeug befragt diesen Store. Beide Variablen sind also korrekt geladen und **innerhalb des Stores selbst** durchgesetzt, wirken sich aber noch auf **keinen tatsächlichen Werkzeugaufruf** aus — das verdrahtet erst der nächste Schritt desselben Plans. Die folgende Beschreibung schildert den vorgesehenen Schutz, nicht bereits heute aktives Verhalten eines laufenden Servers.
