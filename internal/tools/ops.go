...
func registerOpsTools(s *mcp.Server, p *clientpool.Pool, logger *slog.Logger) {
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Check whether this server can currently reach Fileee and log in with the " +
			...
			"Fileee's own account lock, reusing the cached result for calls inside that window, " +
			"and it never returns the counterparty's own error text — only this fixed " +
			"classification.",
		Annotations: &mcp.ToolAnnotations{ReadOnlyHint: true},
