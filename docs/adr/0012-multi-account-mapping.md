...
**Was das ausdrücklich nicht ist.** Keine Entscheidung, Gangway zu übernehmen — die existiert bislang nicht als eigenes ADR. Solange sie nicht getroffen ist, gelten Punkt 2 und 3 unverändert für den heutigen, direkt auf dem SDK aufbauenden Code; sie sind nicht falsch, nur an eine Code-Schicht gebunden, die durch Gangway ersetzbar wäre. Erst mit einer eigenen Entscheidung zur Gangway-Übernahme wird dieses ADR formal überarbeitet: Header-Feld `Überarbeitet durch` hier ergänzt (Gegenstück `Überarbeitet` im neuen ADR), und Punkt 2/3 umformuliert auf „Identität ausschließlich über `serve.IdentityFrom(ctx)`, niemals selbst zwischenspeichern".
